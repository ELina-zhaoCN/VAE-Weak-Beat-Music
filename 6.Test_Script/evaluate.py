...
try:
    ...
    sys.path.append(str(Path(__file__).parent / '../2.Music_dataset'))
    sys.path.append(str(Path(__file__).parent / '../3.Model'))
    sys.path.append(str(Path(__file__).parent / '../4.Beat_losses'))    
    # Import VAE model
    try:
...
def load_model(checkpoint_path: str, latent_dim: int, device: torch.device) -> torch.nn.Module:
    """Load trained model from checkpoint."""
    print(f"Loading model from: {checkpoint_path}")
    
    # Create model instance
    model = MelSpectrogramVAE(latent_dim=latent_dim).to(device)
...
def evaluate_model(model: MelSpectrogramVAE,
    ...
    with torch.no_grad():
        for i in range(min(num_samples, len(dataset))):
            ...
            if reconstructed_mel.size(-1) != original_mel.size(-1):
               reconstructed_mel = reconstructed_mel[..., :original_mel.size(-1)]
            # Compute MSE
